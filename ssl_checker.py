#!/usr/bin/env python3
import socket # For creating a network socket to connect to a host.
import sys
import json
...
class Clr: #This class defines colors to be used when printing messages to the terminal for better readability (reset, red, green, yellow).
    """Text colors."""
...
#It creates a socket connection, then uses SSL to initiate a TLSv1 handshake.[transport layer security]
...
#socket is closed after handshake 
        return cert

    def border_msg(self, message):
        """Print the message in the box for Success or failure message ."""
        row = len(message)
        h = ''.join(['+'] + ['-' * row] + ['+'])
        result = h + '\n' "|" + message + "|"'\n' + h
        print(result)
...
#interact with external API from ssllabs 
        ...
        if endpoint_data['statusMessage'] == 'Certificate not valid for domain name':
            return context
#after analysis return information for ssl vulneribilities 
    ...
    def get_cert_sans(self, x509cert):
        """
        Get Subject Alt Names from Certificate. taken from stack overflow:
    ...
    def get_cert_info(self, host, cert):
        ...
        context = {}
#gathers information of certificate,ie domain it's issued for, the issuing authority, the certificate's validity period, whether it's expired, etc.
...
# gathers info from get_cert _info , process each host by calling get_cert 
        ...
        if not user_args.json_true:
            ...
            if user_args.summary_true:
                ...
                return
#if csv_enabled flag is set it export results as csv files 
        # CSV export if -c/--csv is specified
        if user_args.csv_enabled:
            self.export_csv(context, user_args.csv_enabled, user_args)
#if html_enabled flag is set it export results as html files 
    ...
    def filter_hostname(self, host):
        ...
        return host, port
#defines cammand line arguemts that script will accept , uses argparse module to specify diffrent options 
...
if __name__ == '__main__':
    ...
    #connects to the host via ssl and retrives certificate and extracts varius details such as issuer , validity and subject alternative name (SANs)
    #analyze cert ssl grade and vulneribilty and known issues like POODLE , HEARTbleed, FREAK using ssl labs api
    #warning generate for certthat are close to expiry within 15 days 
    #total number of valid expired warning and failed hosts is reported at the end 
